...
const tradingSkills = pgTable("trading_skills", {
  ...
  // 고유 코드 (예: "golden_cross")
  category: text("category").notNull(),
  // "entry" | "exit" | "signal" | "risk"
  description: text("description"),
  // 스킬 설명
  icon: text("icon"),
  // 이모지 아이콘
  paramsSchema: text("params_schema"),
  // JSON: 파라미터 정의 스키마
...
const userSkillInstances = pgTable("user_skill_instances", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").notNull(),
  skillId: integer("skill_id").notNull(),
  // references trading_skills
  label: text("label"),
  // 사용자 별칭 (예: "삼성전자 골든크로스")
  stockCode: text("stock_code"),
  // 종목코드 (null이면 전체 대상)
  ...
  // 실행 우선순위 (낮을수록 우선)
  status: text("status").default("active"),
  // "active" | "triggered" | "paused" | "completed" | "error"
...
const skillExecutionLogs = pgTable("skill_execution_logs", {
  id: serial("id").primaryKey(),
  instanceId: integer("instance_id").notNull(),
  // references user_skill_instances
  userId: integer("user_id").notNull(),
  skillCode: text("skill_code").notNull(),
  stockCode: text("stock_code"),
  stockName: text("stock_name"),
  action: text("action").notNull(),
  // "check" | "trigger" | "order" | "error"
  detail: text("detail"),
  // 실행 상세 설명
  currentPrice: numeric("current_price"),
  // 체크 시점 현재가
  indicatorValues: text("indicator_values"),
  // JSON: 지표 값들 (rsi, ma, macd 등)
  orderResult: text("order_result"),
  // JSON: 주문 결과
  createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull()
});
const multiFactorStrategy = pgTable("multi_factor_strategy", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").notNull(),
...
const insertMultiFactorStrategySchema = createInsertSchema(multiFactorStrategy).omit({ id: true, createdAt: true, updatedAt: true });
const multiFactorPositions = pgTable("multi_factor_positions", {
  id: serial("id").primaryKey(),
  strategyId: integer("strategy_id").notNull(),
  userId: integer("user_id").notNull(),
  stockCode: text("stock_code").notNull(),
...
const insertMultiFactorPositionSchema = createInsertSchema(multiFactorPositions).omit({ id: true, openedAt: true });
const multiFactorLogs = pgTable("multi_factor_logs", {
  id: serial("id").primaryKey(),
  strategyId: integer("strategy_id").notNull(),
  positionId: integer("position_id"),
  action: text("action").notNull(),
...
const insertMultiFactorLogSchema = createInsertSchema(multiFactorLogs).omit({ id: true, createdAt: true });
export {
  aiPrompts,
  aiReports,
  autoTradeRules,
  ...
  insertMultiFactorLogSchema,
  insertMultiFactorPositionSchema,
  insertMultiFactorStrategySchema,
  insertNoticeSchema,
